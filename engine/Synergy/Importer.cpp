...
Node3D* Importer::ImportNodeFromMemory(char* data, int size) {
    ...
    const aiScene* scene = importer.ReadFileFromMemory(
        data,                   // char* buffer containing the model data
        size,               // size of the model data buffer
    ...
    for (int i = 0; i < scene->mNumMaterials; i++) {
        ...
        if (data.textureFilePaths.size() > 0) {
            ...
            std::string name = PathHelper::ExtractFileName(tpath);
...
Node3D* Importer::ImportNodeFromResource(Resource* resource) {

    return ImportNodeFromMemory(resource->GetData(), resource->GetSize());
